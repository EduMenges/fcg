...
int main() {
    ...
    // Carregamento de todas as funções definidas por OpenGL 3.3, utilizando a
    // biblioteca GLAD.
    ...
    // Habilitamos o Backface Culling. Veja slides 23-34 do documento Aula_13_Clipping_and_Culling.pdf e slides 112-123
    ...
    while (glfwWindowShouldClose(window) == 0) {
        ...
        {
            model = model * Matrix_Translate(-0.55f, 0.0f,
                                             0.0f);  // Atualizamos matriz model (multiplicação à direita) com uma
                                                     // translação para o braço direito
            PushMatrix(model);                       // Guardamos matriz model atual na pilha
            {
                ...
                PushMatrix(model);                   // Guardamos matriz model atual na pilha
                {
                    model = model * Matrix_Scale(0.2f, 0.6f,
                                                 0.2f);  // Atualizamos matriz model (multiplicação à direita) com um
                                                         // escalamento do braço direito
                ...
                PushMatrix(model);  // Guardamos matriz model atual na pilha
                {
                    model = model * Matrix_Translate(0.0f, -0.65f,
                                                     0.0f);  // Atualizamos matriz model (multiplicação à direita) com a
                                                             // translação do antebraço direito
                    model =
                    ...
                    {
                        model = model * Matrix_Scale(0.2f, 0.6f,
                                                     0.2f);  // Atualizamos matriz model (multiplicação à direita) com
                                                             // um escalamento do antebraço direito
                        glUniformMatrix4fv(model_uniform, 1, GL_FALSE,
                                           glm::value_ptr(model));  // Enviamos matriz model atual para a GPU
            ...
            PushMatrix(model);
            {
                model = model * Matrix_Rotate_Z(angleZ_) * Matrix_Rotate_Y(angleY_) * Matrix_Rotate_X(-angleX_);
                PushMatrix(model);
                {
            ...
            PushMatrix(model);
            {
                ...
                PushMatrix(model);
                {
                    ...
                    model = model * Matrix_Rotate_Z(-g_ForearmAngleZ) * Matrix_Rotate_X(g_ForearmAngleX);
                    PushMatrix(model);
                    {
                        model = model * Matrix_Scale(0.2f, 0.6f, 0.2f);
                        glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model));
            ...
            model = model * Matrix_Translate(-0.35f, -1.05f, 0.0f);
            PushMatrix(model);
            {
                PushMatrix(model);
                {
                    model = model * Matrix_Scale(0.3f, 0.6f, 0.3);
                    glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model));
                    DrawCube(render_as_black_uniform);
                }
                PopMatrix(model);
            ...
            model = model * Matrix_Translate(-0.40f, 0.0f, 0.0f);
            PushMatrix(model);
            {
                PushMatrix(model);
                {
        ...
        // "shader_vertex.glsl".
        glUniform1i(render_as_black_uniform, GLFW_FALSE);
...
void DrawCube(GLint render_as_black_uniform) {
    ...
    glDrawElements(g_VirtualScene["cube_faces"]
                   ...
                   GL_UNSIGNED_INT, g_VirtualScene["cube_faces"].first_index);
    ...
    // "shader_vertex.glsl".
    glUniform1i(render_as_black_uniform, GLFW_TRUE);
...
GLuint BuildTriangles() {
    ...
    //
    // Este vetor "model_coefficients" define a GEOMETRIA (veja slides 103-110 do documento
    ...
    // cada vértice: uma cor (veja slides 109-112 do documento Aula_03_Rendering_Pipeline_Grafico.pdf e slide 111 do
    ...
    // model_coefficients.
    //
    // Para referência sobre os modos de renderização, veja slides 182-188 do documento
    // Aula_04_Modelagem_Geometrica_3D.pdf.
    //
...
void CursorPosCallback(GLFWwindow* window, double xpos, double ypos) {
    ...
    if (g_RightMouseButtonPressed) {
        ...
        // Atualizamos as variáveis globais para armazenar a posição atual do
        // cursor a última posição conhecida do cursor.
        g_LastCursorPosX = xpos;
        g_LastCursorPosY = ypos;
    }
