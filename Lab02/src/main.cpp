...
int main() {
	...
	GLint projection_uniform = glGetUniformLocation(program_id,
	...
	while (!glfwWindowShouldClose(window)) {
		...
		if (isMovingForward) cameraPos = cameraPos + (cameraTarget*kCameraSpeed);
		if (isMovingBackward) cameraPos = cameraPos - (cameraTarget*kCameraSpeed);
		if (isMovingRight) cameraPos = cameraPos - (cameraRight*kCameraSpeed);
		if (isMovingLeft) cameraPos = cameraPos + (cameraRight*kCameraSpeed);
...
void ScrollCallback(GLFWwindow *window, double xoffset, double yoffset) {
	...
	g_CameraDistance -= static_cast<float>(0.1*yoffset);
